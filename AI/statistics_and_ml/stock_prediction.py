...
def get_signal(current_price, prices):
    # Assuming you have current_price separate from historical prices
    if current_price < median(prices):
        return "BUY"  # Price dipped below historical center
    elif current_price > median(prices): 
        return "SELL" # Price above historical center

    
def calculate_num_of_stocks_to_buy(days_to_trade): 
    print    


def stock_prediction(text): 
    data = text.splitlines()

    amount_to_spend, num_of_stock_ticker, days_to_trade = map(int, data[0].split()) 

    data = text.splitlines()[1:] #removing first line by reading 
    portfolio = []
    for line in data: 
        ticker, owned, *price = line.split()
        owned = int(owned)
        ...
        portfolio.append({
            ...
            'owned' : owned,
            'prices' : prices,
            'signal' : get_signal(prices[-1], prices)
        })
