...
def cema(prices):
    # Custom Exponential Moving Average (weights recent prices more)
    weights = [0.163, 0.294, 0.353, 0.294, 0.163]
...
def printTransactions(money, k, d, name, owned, prices):
    ...
    stock_signals = []
    for i in range(k):
        curr_price = prices[i][-1]  # Today's price (last in 5-day window)
        sma = sum(prices[i]) / 5    # Simple moving average
        signal = cema(prices[i]) - sma  # Key metric: momentum vs trend
        
        stock_signals.append((name[i], owned[i], curr_price, signal, i))
    
    # Find best SELL candidate (highest positive signal + shares owned)
    sell_candidates = [(s[3], s[0], s[1], s[2]) for s in stock_signals 
                      if s[1] > 0 and s[3] > 0]  # (signal, name, owned, price)
    if sell_candidates:
        best_sell = max(sell_candidates)
        transactions.append(f"{best_sell[1]} SELL {int(best_sell[2])}")
    
    # Find best BUY candidate (lowest negative signal)
    buy_candidates = [(s[3], s[0], s[2]) for s in stock_signals if s[3] < 0]  # (signal, name, price)
    if buy_candidates and len(transactions) < 2:  # Max 2 transactions
        best_buy = min(buy_candidates)
        shares = int(money // best_buy[2])  # How many can afford
        if shares > 0:
            transactions.append(f"{best_buy[1]} BUY {shares}")
...
# Interactive main loop - HackerRank calls this repeatedly
    ...
    while True:
        try:
            ...
            for _ in range(k):
                ...
                prices_list.append(list(map(float, line[2:])))
            
            # Call the required function
            printTransactions(m, k, d, names, owned_list, prices_list)
